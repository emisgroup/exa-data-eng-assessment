...
print(pd.DataFrame(patient_data, columns=['id', 'passport_no', 'driving_license', 'social_security_no', 'medical_record_no', 'name', 'telecom', 'gender', 'birth_date', 'deceased_date_time', 'address', 'marital_status', 'communication']))
print(pd.DataFrame(encounter_data, columns=['id', 'status', 'class_code', 'type_code', 'type_text', 'subject', 'start_date', 'end_date', 'location', 'service_provider']))
print(pd.DataFrame(condition_data, columns=['id', 'clinical_status', 'verification_status', 'category_code', 'category', 'subject', 'encounter', 'onset_recorded', 'recorded_date', 'code', 'text']))
print(pd.DataFrame(diagnostic_data, columns=['id', 'status', 'category', 'subject', 'encounter', 'effective_date', 'issued_date', 'performer']))
print(pd.DataFrame(document_reference, columns=['id', 'status', 'type', 'category', 'subject', 'date', 'author', 'custodian', 'encounter', 'period_start', 'period_end']))
print(pd.DataFrame(claim_data, columns=['id', 'status', 'type_code', 'patient', 'billable_start_date', 'billable_end_date', 'created_date', 'provider', 'priority', 'insurence', 'service_codes', 'total_amount']))
print(pd.DataFrame(procedure_data, columns=['id', 'status', 'code', 'text', 'subject', 'encounter', 'performed_start_date', 'performed_end_date', 'location']))
print(pd.DataFrame(immunization_data, columns=['id', 'status', 'vaccine_code', 'vaccine_name', 'patient', 'encounter', 'occurrence_date_time', 'primary_source', 'location']))
print(pd.DataFrame(eob_data, columns=['id', 'status', 'patient', 'insurer', 'claim_id', 'total_amount', 'payment_amount']))
